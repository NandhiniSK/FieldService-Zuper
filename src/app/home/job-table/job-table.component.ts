...
import { JobPipe } from 'src/app/shared/pipes/job-filter.pipe';
...
export class JobTableComponent implements OnInit {
  ...
  ngOnChanges(){
    let filterList = this.jobFilterPipe.transform(this.jobList, this.filterType, this.filterVal);
  ...
  onFilterChange(filterList){
    if(this.sortable.isAscending)
      this.filteredList = filterList.sort((a, b) => a[this.sortable.columnName] < b[this.sortable.columnName] ? -1 : a[this.sortable.columnName] > b[this.sortable.columnName] ? 1 : 0)
    else
      this.filteredList = filterList.sort((a, b) => a[this.sortable.columnName] > b[this.sortable.columnName] ? -1 : a[this.sortable.columnName] < b[this.sortable.columnName] ? 1 : 0)
